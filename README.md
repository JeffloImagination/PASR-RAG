...
The repository already contains experiment outputs under `outputs/results/`.

For example, `outputs/results/main_experiment_matrix_full50_llm_local_summary_prompt_v2/matrix_summary.json` records a 50-example comparison among PASR-RAG variants and FlashRAG baselines. This makes the repository suitable both for reproduction and for extension experiments.

## Notes for GitHub Publishing

- The repository currently contains large local assets under `models/`, `data/`, and `outputs/`.
- If you plan to publish it publicly, consider trimming generated artifacts or managing large files with Git LFS.
- The `FlashRAG/` directory appears to be a vendored dependency. If you keep it in the repo, document its origin and license clearly.

## Citation

If you use this repository in academic work, please cite the corresponding thesis, paper, or project page when available.

## License

No top-level project license file is currently present in the repository root. Add a license before publishing to GitHub if you want others to reuse the code legally.
